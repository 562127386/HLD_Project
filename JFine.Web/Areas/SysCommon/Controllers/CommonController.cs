...
namespace JFine.Web.Areas.SysCommon.Controllers
{
    public class CommonController : JFControllerBase2
    {
        ...
        [HttpGet]
        [HandlerLogin]
        public ActionResult UpLoadExcelOrderData()
        {
            return View();
        }
        /// <summary>
        ...
        public JsonResult Upload_Order_Excel()
        {
            Hashtable ht_result = new Hashtable();
            List<StringBuilder> sqls = new List<StringBuilder>();
            List<object> objs = new List<object>();
            string tablename = "IOT_Order_Sec";
            string virPath = ConstantUtil.FILE_TEMP_URL;
            ExcelHelper excel = new ExcelHelper();
            DataTable dt = excel.upExcelToDatatable(HttpContext, virPath, ref ht_result);
            if (dt != null)
            {
                DataRow[] arrayDR1 = dt.Select("ProductLine is null or ProductLine='' or OrderName is null or OrderName='' or PCode is null or PCode='' ");
                if (arrayDR1 != null && arrayDR1.Length > 0)
                {
                    ht_result["status"] = "F";
                    ht_result["msg"] = "导入失败，含有产线、订单编码、型号、名称为空的数据，请完善！";
                    return Json(ht_result);
                }
                List<Hashtable> order_list = DataTableHelper.DataTableToList(dt);
                List<Hashtable> keylist = new List<Hashtable>();
                var userId = OnlineUser.Operator.GetCurrent().UserId;
                var userName = OnlineUser.Operator.GetCurrent().UserName;
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    string Id_Key = SequenceHelper.getSnowflakeID();
                    order_list[i].Add("Id", Id_Key);
                    order_list[i].Add("QualifyQuantity", "0");
                    order_list[i].Add("UnqualifyQuantity", "0");
                    order_list[i].Add("ProceedStatus", "0");
                    order_list[i].Add("Status", "0");
                    order_list[i].Add("StartStatus", "0");
                    order_list[i].Add("CreateDate", DateTime.Now);
                    order_list[i].Add("CreateUserId", userId);
                    order_list[i].Add("CreateUserName", userName);
                    Hashtable key_ht = new Hashtable();
                    key_ht.Add("Id", Id_Key);
                    keylist.Add(key_ht);
                }
                //生成子表sql
                SqlUtil.getBatchSqls(order_list, tablename, keylist, ref sqls, ref objs);
                //执行sql
