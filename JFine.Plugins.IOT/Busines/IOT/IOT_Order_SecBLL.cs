    ...
    public class IOT_Order_SecBLL
    {
        ...
        public IEnumerable<IOT_Order_SecEntity> GetPageList(Pagination pagination, string queryJson)
        {
            ...
            if (queryJson.IsNullOrEmpty())
            {
                ...
                expression = expression.And(t => t.OrderDate == dateNow);
            }

            //查询条件
            if (!queryParam["orderDate_Begin"].IsEmpty())
            {
                string orderDate_Begin = queryParam["orderDate_Begin"].ToString();
                DateTime? orderDate_Begin_Date = DateTime.Parse(orderDate_Begin);
                expression = expression.And(t => t.OrderDate >= orderDate_Begin_Date);
            }
            if (!queryParam["orderDate_End"].IsEmpty())
            {
                string orderDate_End = queryParam["orderDate_End"].ToString();
                DateTime? orderDate_End_Date = DateTime.Parse(orderDate_End);
                expression = expression.And(t => t.OrderDate <= orderDate_End_Date);
            }
            if (!queryParam["searchText"].IsEmpty())
            {
                string searchText = queryParam["searchText"].ToString();
                expression = expression.And(t => t.PCode.Contains(searchText) || t.ProductLine == searchText);
            }
            if (!queryParam["PCode"].IsEmpty())
            {
                string PCode = queryParam["PCode"].ToString();
                expression = expression.And(t =>t.PCode.Contains(PCode));
            }
            if (!queryParam["ProductLine"].IsEmpty())
            {
                string ProductLine = queryParam["ProductLine"].ToString();
                expression = expression.And(t =>t.ProductLineName.Contains(ProductLine));
            }
        ...
        public void FinishProceedOrder(string keyValue)
        {
            try
            {
                service.FinishProceedOrder(keyValue);
                CacheFactory.Cache().RemoveCache(cacheKey);
            }
            catch (Exception)
            {
        ...
        public void UpdateOrderQuantity(string keyValue, string quantityType,string lineCode="")
        {
            try
            {
                service.UpdateOrderQuantity(keyValue, quantityType,lineCode);
                CacheFactory.Cache().RemoveCache(cacheKey);
            }
            catch (Exception)
            {
        ...
        {
          return  service.GetProceedOrder();
        }
        #endregion
